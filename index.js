...
var gifImage = function (width, height, backgroundColor, foregroundColor) {
  ...
  _this.hexStringToArray = function (str) {
    if (!/^#[0-9a-fA-F]{6}$/.test (str)) {
      console.error (
        '[qrcode-base64-size::hexStringToArray] Invalid input. The input should be in the format like "#123456".'
      );
      return null;
    }
...
var drawImg = function (text, options) {
  ...
  // 调整偏移量计算,为 0 则没有白边边距问题，默认为 2*2
  var codePadding = dealDefaultValue (options.codePadding);
  ...
  var MARGIN_COUNT = codePadding;
  var cellsize = size / (qr.getModuleCount () + MARGIN_COUNT);
